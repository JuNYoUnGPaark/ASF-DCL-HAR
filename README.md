...
We further validate the feasibility of the ASF framework on a Raspberry Pi 3B+ edge platform, suggesting that the proposed model can run fully on-device with practical low-latency inference and estimated energy efficiency suitable for real-time wearable HAR. Notably, INT8 quantization reduces the model size by about 71\% with only marginal performance degradation, while also improving runtime and energy efficiency, supporting ASF as a practical solution for efficient edge deployment.
